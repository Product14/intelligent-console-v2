...
import { generateBearerToken, Logout } from "@spyne-console/utils/config";

export const handlePatchRequest = (URL, callParams, headers = {}, cancelToken = axios.CancelToken.source(), bodyData = {}) => {
    let bearerToken = ''    
    ...
    let handlePatchPromise = new Promise((resolve, reject) => {
        axios({
            ...
            cancelToken: cancelToken ? cancelToken.token : axios.CancelToken.source().token,
            data: bodyData
        }).then(res => {
            if(res?.data?.error) {
