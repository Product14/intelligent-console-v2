...
const upsertQueryParams = async ({variable=null,value=null,deleteVariable = false,deleteElementInsideArray = false,isAList=false,router}) => {
                    ...
                    if(deleteElementInsideArray){
                      ...
                      if(typeof(value) === 'number'){
                        ...
                        if(elements.indexOf(value) >= 0){
                           let newList = [...elements.slice(0,elements.indexOf(value)),...elements.slice(elements.indexOf(value)+1)]                           
                           if(newList.length === 0){
                            queryParams.delete(variable)
                           }else{
...
const getListOfValuesFromQueryParam = (value) => {
  return value?.substring(value.indexOf('[')+1,value.indexOf(']')).split(',')
}
...
const handleColumnFiltersFormat = (columnFilters) => {
    ...
    for(const key in columnFilters){
        if(listOfLikeColumns.includes(key)){
            requiredDataFormat.push({"key": key,"value":columnFilters[key], "type":"like"})
        }else if(listOfDateColumns.includes(key)){
            requiredDataFormat.push({"key": key,"value":columnFilters[key], "type":"date"})
        }
        else if(typeof columnFilters[key] === "object"){
            requiredDataFormat.push({"key": key,"value": columnFilters[key], "type":"list"})
        }
        else if(typeof columnFilters[key] === "string"){
            requiredDataFormat.push({"key": key,"value": columnFilters[key], "type":"field"})
        }
    }
...
const convertBlobToFile = (blobFileObj) => {
    try{
        return new Promise((resolve, reject)=>{
            if(!blobFileObj || !blobFileObj['name'] || !blobFileObj['blobUrl'] || !blobFileObj['type'] || !blobFileObj['lastModified']){
              return reject('File data required to convert to file is not sufficient');
            }
...
const compressFileObject = ({file, imageData={'name':'', 'type':'', 'lastModified':''}, imageKey, quality=0.6})=>{
  return new Promise((resolveCompressoion, _)=>{
    new Compressor(file, {
      ...
      success(result) {
        ...
        resolveCompressoion({'file':newFile, imageKey} ?? {'file':result, imageKey});
      },
      error(err) {
