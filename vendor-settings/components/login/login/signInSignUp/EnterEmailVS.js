...
function EnterEmailVS(props) {
  ...
  const { ENTER_EMAIL_DATA, allowClose, source = 'browser' } = props;
  ...
  return (
    <div className="lsm:w-12/12 flex w-full flex-col justify-start">
      <form
        className={`${loginType != 'home-page' ? 'max-tb:px-5' : 'hidden'} `}
      >
        <div className="lsm:text-3xl flex items-center gap-2 text-left text-2xl font-bold text-[#402387]">
          <Image
            onClick={() => {
        ...
        <HideSpyneContent>
          <p className="lsm:mb-8 text-black-60 mb-6 mt-3 text-sm font-normal">
            {showEmailFields ? (
              loginOrSignup === 'login' ? (
                errorMsg ? (
                  'This email is not registered yet, please signup to continue'
                ) : (
        ...
        {showEmailFields ? (
          <>
            <div className="newinput-wrapper relative mb-8">
              <input
                ...
                onBlur={(e) => {
                  ...
                  if (emailPattern.test(e.target.value)) {
                    ...
                    setErrorMsg(
                      translate(
                        'console.screens.emailScreen.invalidEmailError2'
                      )
                    );
                  }
                }}
              ...
              {/* <div className="text-sm ml-3 pt-1">{translate('console.screens.emailScreen.validEmailWarning')}</div> */}
            </div>
            ...
            <button
              className={[
                SignInSpinner ? '' : '',
                'secondary-btn !h-12 w-full !py-1.5 !font-medium',
              ].join(' ')}
              type="submit"
            ...
            <HideSpyneContent>
              <div className="mt-3">
                {loginOrSignup == 'login' ? (
                  ...
                  >
                    {translate(
                      'console.screens.signInSignUpScreen.existingUser'
                    )}{' '}
          ...
          <>
            <div className="flex flex-col gap-4">
              <HideSpyneContent>
                ...
                />
                {errorMsg ? (
                  <p className="text-red text-xs">{errorMsg}</p>
                ) : null}
              </HideSpyneContent>
              <button
                onClick={() => {
                  ...
                  captureEvent(
                    ...
                    {
                      method: 'email',
                      login_flow: allowClose
                        ? 'non_restrictive'
                        : 'restrictive',
                      source: getDemoFlowType(
                        mainRoute,
                        childRoute,
                        queryParam
                      ),
              ...
              >
                <Image
                  ...
                  className=""
                />
                {translate('console.screens.signInSignUpVS.emailLabel')}
              </button>
            </div>
        ...
        {(loginOrSignup === 'login' || !showEmailFields) && (
          <div className="mt-10 text-center">
            ...
            <span
              ...
              class="font-['Inter'] text-sm font-semibold leading-tight text-[#5925db]"
            >
              {loginOrSignup === 'login'
                ? translate('console.screens.signInSignUpVS.signUp')
                : translate('console.screens.signInSignUpScreen.login')}
