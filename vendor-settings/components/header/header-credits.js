...
export default function HeaderCredits({ setOpenUpgradeModal }) {
  ...
  const closeCreditDropDown = (e) => {
    e.stopPropagation();
    // capture_plan_info_clicked(inventoryDetails?.dealerVinId, inventoryDetails?.mediaId)
  ...
  const renderCreditsDropdownContent = () => (
    <div className="border-black-10 absolute right-0 top-14 z-[2300] min-w-[22.5rem] max-w-[24rem] rounded-lg border bg-white shadow-lg">
      {showGlobalUpgrade(credits?.credits_v5) && !isNewPricingPlan && (
        <div className="flex flex-col gap-4 rounded-t-lg bg-gray-100 p-[20px] px-4">
          <div
            className={`${styles['console-logo']} flex justify-between py-[11px]`}
          >
            <img
      ...
      <div className="px-5 py-1">
        {freeCreditsInfo?.map((item, idx) => (
          <div
            className="border-black-10 flex items-center justify-between gap-2 border-b-[0.6px] py-4"
            key={`${item?.product}`}
          >
            <div className="flex items-center justify-between gap-3 align-middle">
              <div>
                <p className="text-black-80 flex items-center gap-2 text-sm font-medium">
                  <Image
                    src={item.icon}
                    alt={item.product}
                    width={15}
                ...
                {!isPaidProduct(credits?.credits_v5, item?.key) && (
                  <p className="mt-3 font-['Inter'] text-xs font-medium leading-none text-black/40">
                    {formatNumber(item?.value)} {item?.title}
                  </p>
                )}
              </div>
            </div>
            {!isGoldProduct(credits?.credits_v5, item?.key) &&
              !isComprehensiveProduct(credits?.credits_v5, item?.key) &&
              !isProProduct(credits?.credits_v5, item?.key) && (
                <div
        ...
        {showGlobalUpgrade(credits?.credits_v5) && !isNewPricingPlan && (
          <button
            ...
            className="text-blue-light px-4 py-4 text-xs font-semibold"
          >
            View Pricing
          </button>
        )}
  ...
  if (!credits?.credits_v5?.credit_data) {
    return (
      <div className="relative flex flex-row items-center">
        <div className="mr-3 flex h-[38px] animate-pulse items-center rounded-[8px] bg-gray-200 md:mr-0">
          <div className="hidden flex-row items-center pl-1 md:flex lg:pl-3">
            ...
            <div className="ml-2 h-4 w-16 animate-pulse rounded bg-gray-300"></div>
          </div>
        </div>
      </div>
    );
  ...
  const shouldShowPlanBadgeCard =
    credits?.credits_v5?.overall_stage === 'Pro' ||
    credits?.credits_v5?.overall_stage === 'Lite';

  return (
    <div className="relative flex flex-row items-center">
      <div className="relative" ref={creditsDropdownRef}>
        {shouldShowPlanBadgeCard ? (
          <PlanVisibilityBadgeCard
            data={credits?.credits_v5}
