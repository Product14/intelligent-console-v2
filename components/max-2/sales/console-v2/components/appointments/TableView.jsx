    ...
    <th className="px-4 py-2.5 text-left">
      ...
      >
        ...
        {active ? (
          ...
          <MaterialSymbol name="unfold_more" size={14} className="text-[#d1d5db]" />
        )}
      </button>
    </th>
  );
...
function PlainTh({ label }) {
  return (
    <th className="px-4 py-2.5 text-left text-[10px] font-bold uppercase tracking-wider text-[#6b7280]">
      {label}
    </th>
  );
}
...
export default function TableView({ meetings, service, onSelect }) {
          ...
          {rows.map((m, i) => {
            ...
            return (
              <tr
                key={m.id || i}
                onClick={() => onSelect(m)}
                className="cursor-pointer border-b border-[#f5f5f5] last:border-0 transition-colors"
                style={{ "--hover-bg": "#faf8ff" }}
              ...
              >
                ...
                <Td>
                  <div className="truncate text-[13px] font-bold text-[#1a1a1a]">{customerName(m)}</div>
                  {phone && (
                    <a
                      href={`tel:${phone}`}
                  ...
                  <Td>
                    {services.length > 0 ? (
                      <span className="text-[12px] text-[#374151]">
                        {services[0]}
                        {services.length > 1 && (
                          <span className="text-[#9ca3af]"> +{services.length - 1}</span>
                        )}
                      </span>
                    ) : (
                      <span className="text-[12px] text-[#9ca3af]">—</span>
